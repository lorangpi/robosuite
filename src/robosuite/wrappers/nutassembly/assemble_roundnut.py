...
class AssembleRoundNutWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        state = self.detector.get_groundings(as_dict=True, binary_to_float=False, return_distance=False)
        success = state[f"on(roundnut,squarepeg)"]
    ...
    def staged_rewards(self, state):
        ...
        active_nuts = [self.env.nuts[1]] # corresponds to the round nut
            ...
            dists = np.array(
                [
                    np.linalg.norm(
                        ...
                        - self.sim.data.body_xpos[self.peg1_body_id]
                    )
        ...
        is_grasped = state["grasped(roundnut)"]
        r_grasp = (
            int(
                is_grasped
            )
