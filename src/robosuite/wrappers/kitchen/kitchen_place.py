...
class KitchenPlaceWrapper(gym.Wrapper):
    ...
    def reset(self, seed=None):
        ...
        if self.obj_to_pick == "pot":
            self.place_to_drop = np.random.choice(["stove", "serving"])
        else:
            self.place_to_drop = "pot"
        self.step_count = 0
        success_reset = False
    ...
    def staged_rewards(self, state):
        ...
        elif state[f"picked_up({self.obj_to_pick})"] or self.obj_to_pick_up:
            drop_pos = self.env.sim.data.body_xpos[self.env.sim.model.body_name2id(self.detector.object_id[self.place_to_drop])][:2]
            gripper_pos = self.env.sim.data.body_xpos[self.gripper_body][:2]
            dist = np.linalg.norm(drop_pos - gripper_pos)
            reward = -6 - (np.tanh(10.0 * dist))
            self.obj_to_pick_up = True
        elif not(state[f"grasped({self.obj_to_pick})"]):
            self.obj_to_pick_up = False
